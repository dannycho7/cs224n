...
def naiveSoftmaxLossAndGradient(
    ...
    gradOutsideVecs = np.expand_dims(y_hat - y, axis=1) * centerWordVec
    ### END YOUR CODE
...
def negSamplingLossAndGradient(
    ...
    sigmoids = {i: sigmoid((1 if (i == outsideWordIdx) else -1) * np.dot(outsideVectors[i], centerWordVec)) for i in set(indices)}
    loss = sum(-np.log(sigmoids[i]) for i in indices)
    gradCenterVec = sum((-1 if (i == outsideWordIdx) else 1) * outsideVectors[i] * (1 - sigmoids[i]) for i in indices)
    gradOutsideVecs = np.zeros(outsideVectors.shape)
    for i in indices:
        gradOutsideVecs[i] += centerWordVec * (-1 if (i == outsideWordIdx) else 1) * (1 - sigmoids[i])
    ### END YOUR CODE
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for outsideWord in outsideWords:
        j = word2Ind[outsideWord]
        l, gcv, gov = word2vecLossAndGradient(centerWordVec, j, outsideVectors, dataset)
        loss += l
